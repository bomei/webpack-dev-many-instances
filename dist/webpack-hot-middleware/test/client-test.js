...
describe("client", function () {
  ...
  context("with default options", function () {
    ...
    it("should show overlay on errored builds", function () {
      ...
      sinon.assert.calledWith(clientOverlay.showProblems, "errors", ["Something broke", "Actually, 2 things broke"]);
    });
  ...
  context("with overlayWarnings: true", function () {
    beforeEach(function setup() {
      global.__resourceQuery = '?overlayWarnings=true';
      global.document = {};
      global.window = {
        EventSource: sinon.stub().returns({
          close: sinon.spy()
    ...
    it("should show overlay on warning builds", function () {
      ...
      sinon.assert.calledOnce(clientOverlay.showProblems);
      sinon.assert.calledWith(clientOverlay.showProblems, "warnings", ["This isn't great, but it's not terrible"]);
    });
  ...
  context("with name options", function () {
    beforeEach(function setup() {
      global.__resourceQuery = '?name=test';
      global.window = {
        EventSource: sinon.stub().returns({
          close: sinon.spy()
        })
  ...
  function makeMessage(obj) {
    return { data: typeof obj === 'string' ? obj : (0, _stringify2.default)(obj) };
  }
